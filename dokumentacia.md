...
Následne si vygenerujeme pozície, kde chceme vkládať slová. O to sa stará predikát `vytvor_pozicie/4`. Vygenerujú sa pozície smerom `dole` ale aj smerom `doprava`. Tieto pozície sa vygenerujú po ľavom a hornom okraji krížovky. Napríklad pre krížovkou s výškou 3, šírkou 2 miestom, kde je začiatok tajenky je druhý riadok by sa vygenerovali tieto pozície:
...
Toto je popísaná hlavná funkcionalita programu Krížovka. Všetky spomenuté predikáty, ako aj iné pomocné prediáty, sú dôkladne okomentované v zdrojovom kóde. 